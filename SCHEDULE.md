...
| 阶段 | 主题 | 课时 | 难度 | 核心目标 |
|------|------|------|------|---------|
| Phase 1 | 环境配置与基础工具 | 3.5 | ⭐ | 配置开发环境，实现 LLM 客户端和向量检索 |
| Phase 2 | Naive RAG 基线 | 3 | ⭐ | 实现简单 RAG 系统作为对比基线 |
| Phase 3 | Agent-RAG 核心组件 | 7 | ⭐⭐⭐ | 实现 Planner/StepDefiner/Agent/Summarizer 等 |
| Phase 4 | 工作流和主入口 | 3 | ⭐⭐ | 整合各组件为完整工作流，支持命令行运行 |
...
### Phase 4: 工作流和主入口（3 课时）

#### 周期 4.1: LangGraph 工作流（1.5 课时）
